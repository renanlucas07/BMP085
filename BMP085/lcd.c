...
void sendString(char *StringOfCharacters)     // Takes input a string and displays it
...
void sendInteger(int x)     // Takes an integer as input and displays it
{                           // value of integer should be in between
	char buffer[8];         // the range of "int",
	itoa(x,buffer,10);      //  else it'll print garbage values.
	sendString(buffer);		// It use Send_A_String() for displaying.
}
